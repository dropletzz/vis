...
class ManipulationSystem {
  ...
  /**
   * Creatie a new edge starting from given node.
   *
   * @param {Node} node
   */
  addEdgeFromMode(fromId) {
    // when using the gui, enable edit mode if it wasnt already.
    if (this.editMode !== true) {
      this.enableEditMode();
    }
   ...
   * @param {Node} node
   */
  addConnectedNodeMode(fromId, toId) {
    // when using the gui, enable edit mode if it wasnt already.
    if (this.editMode !== true) {
      this.enableEditMode();
    }
  ...
  _controlNodeTouch(event) {
    ...
    let pointerObj = this.selectionHandler._pointerToPositionObject(this.lastTouch);
    let from = this.body.nodes[this.temporaryIds.nodes[0]];
    let to   = this.body.nodes[this.temporaryIds.nodes[1]];

    // exit editMode if something different from a controlNode is touched
    if (!from.isOverlappingWith(pointerObj) && !to.isOverlappingWith(pointerObj)) {
      this.selectionHandler.unselectAll();
      this.showManipulatorToolbar();
      this._clean();
    }
  ...
  _finishConnect(event) {
    ...
    if (node !== undefined) {
      ...
      // exit editMode if there's no node to connect to
      this.selectionHandler.unselectAll();
      this.showManipulatorToolbar();
      this._clean();
    }
   ...
   * @private
   */
  _finishConnectedNodeMode(event) {
    let temporaryEdge = this.body.edges[this.temporaryIds.edges[0]];
  ...
  /**
   * Creates a temporary edge starting from node.
   * Used for graphic purposes when adding edges.
   *
   * @param {Node} node
   * @private
   */
  _createTemporaryEdge(fromId, toId) {
    // create a node the temporary line can look at
    let targetNode;
    if (toId === undefined) {
      let node = this.body.nodes[fromId];
      targetNode = this._getNewTargetNode(node.x,node.y);
      this.body.nodes[targetNode.id] = targetNode;
      this.body.nodeIndices.push(targetNode.id);
    }
    else
      targetNode = this.body.nodes[toId];
